...
const InnerContent = (MenuItems) => {
  ...
  for (let i = 0; i < MenuItems.length; i++) {
    for (let index = 0; index < MenuItems[i].count; index++) {
      ItemDiv.push(<h1 key={v1()}>{MenuItems[i].piece}</h1>)
    }
  }
  if (ItemDiv.length === 0) {
    ItemDiv.push(<div key={v1()} className='py-1 d-flex justify-content-center'><b>No ingrediants Added</b></div>)
  }
  return (ItemDiv)
}
    ...
    <>
      <div className=' pt-2 pb-3 d-flex justify-content-center'>
        <div className='burger-div p-5'>
          <div className='mb-2'><img src={burgerTop} className='burger-image burger-top' alt='Burger-Top' /></div>
          {InnerContent(MenuItems)}
          <div><img src={burgerBottom} className='burger-image burger-bottom' alt='Burger-Top' /></div>
        </div>
      </div>
    </>
